...
export default function Footer({ contacts }: { contacts?: ContactsContent }) {
  ...
  const address = contacts?.address ?? "Махачкала, ул. Хаджи Булача 71";
  const mapUrl = contacts?.mapUrl ?? "https://yandex.ru/maps/-/CHEzfL~r";
  ...
  return (
    <footer className="bg-dark text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-20">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12">
          ...
          <div>
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <div className="text-white font-bold tracking-widest text-sm">ZR AUTO</div>
          ...
          {/* Navigation */}
          <div>
            <h3 className="text-white/60 font-semibold mb-5 text-xs tracking-widest uppercase">Навигация</h3>
            <ul className="space-y-3">
              {[
                ...
                <li key={link.href}>
                  <Link href={link.href} className="text-white/40 hover:text-white text-sm transition-colors duration-200">{link.label}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          {/* Contacts */}
          <div>
            <h3 className="text-white/60 font-semibold mb-5 text-xs tracking-widest uppercase">Контакты</h3>
            <ul className="space-y-3">
              <li>
                <a href={`tel:${phoneRaw}`} className="text-white/40 hover:text-white text-sm transition-colors duration-200 flex items-center gap-2.5">
                  <span className="w-1 h-1 bg-primary rounded-full shrink-0" />{phone}
                </a>
              </li>
              <li>
                <a href={whatsapp} target="_blank" rel="noopener noreferrer" className="text-white/40 hover:text-white text-sm transition-colors duration-200 flex items-center gap-2.5">
                  <span className="w-1 h-1 bg-emerald-500 rounded-full shrink-0" />WhatsApp
                </a>
              </li>
              <li>
                <a href={mapUrl} target="_blank" rel="noopener noreferrer" className="text-white/40 hover:text-white text-sm transition-colors duration-200 flex items-start gap-2.5">
          ...
          {/* Schedule */}
          <div>
            <h3 className="text-white/60 font-semibold mb-5 text-xs tracking-widest uppercase">Режим работы</h3>
            <ul className="space-y-2.5 text-sm">
              {workHours.map((wh) => (
                <li key={wh.days} className="flex justify-between">
                  <span className="text-white/40">{wh.days}</span>
                  <span className="text-white/60">{wh.hours}</span>
                </li>
              ))}
            </ul>
            <div className="mt-6 flex gap-2">
              {socials.map((s) => (
                <a
                  ...
                  className="w-9 h-9 border border-white/10 rounded-lg flex items-center justify-center text-white/30 hover:text-white hover:border-white/30 hover:bg-white/5 transition-all duration-200"
                  aria-label={s.name}
                >
        ...
        <div className="mt-14 pt-8 border-t border-white/[0.06] text-center text-white/25 text-sm">
