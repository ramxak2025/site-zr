      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-6 mb-16">
          ...
          <ScrollReveal animation="fade-right" className="flex items-end lg:justify-end">
            <div className="flex gap-2 sm:gap-3 overflow-x-auto max-w-full pb-1 -mb-1">
              {plats.map((p) => (
                <a key={p.name} href={p.url} target="_blank" rel="noopener noreferrer" className="card px-3 sm:px-5 py-2.5 sm:py-3 group shrink-0">
                  <div className="text-text-secondary text-xs font-medium group-hover:text-text transition-colors whitespace-nowrap">{p.name}</div>
