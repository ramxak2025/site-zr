  ...
  return (
    <div className="flex gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <svg key={i} className={`w-3 h-3 md:w-3.5 md:h-3.5 ${i < count ? "text-amber-400" : "text-zinc-300"}`} fill="currentColor" viewBox="0 0 20 20">
...
export default function Reviews({ items, platforms }: { items?: ReviewItem[]; platforms?: ReviewPlatform[] }) {
  const reviews = items ?? DEFAULT_REVIEWS;
  const plats = platforms ?? DEFAULT_PLATFORMS;

  return (
    <section id="reviews" className="py-16 md:py-28 bg-surface relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 left-1/3 w-[500px] h-[500px] bg-amber-500/[0.03] rounded-full blur-[120px]" />
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 lg:grid-cols-12 gap-3 md:gap-5">
          ...
          <ScrollReveal animation="fade-up" className="col-span-2 md:col-span-6 lg:col-span-12">
            <div className="card p-3 md:p-6">
              <div className="flex items-center justify-center gap-2 md:gap-4">
                {plats.map((p) => (
                  <a key={p.name} href={p.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 md:gap-3 px-3 py-2 md:px-5 md:py-3 rounded-xl md:rounded-2xl bg-surface hover:bg-white border border-border hover:border-primary/20 transition-all group flex-1 md:flex-initial justify-center">
                    <div className="text-center md:text-left">
                      <div className="text-text-secondary text-[10px] md:text-xs font-medium group-hover:text-text transition-colors">{p.name}</div>
                      <div className="flex items-center gap-1 mt-0.5 justify-center md:justify-start">
                        <svg className="w-3 h-3 md:w-4 md:h-4 text-amber-400" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                        <span className="text-text font-black text-sm md:text-lg text-display">{p.rating}</span>
                      </div>
                    </div>
                  </a>
                ))}
          ...
          {/* Featured review — full width mobile */}
          <ScrollReveal animation="scale" className="col-span-2 md:col-span-6 lg:col-span-7">
            <div className="card p-5 md:p-8 h-full relative overflow-hidden group">
              ...
              <div className="relative">
                ...
                <blockquote className="text-text text-base md:text-xl leading-relaxed mt-3 md:mt-5 mb-4 md:mb-6 font-medium">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-text font-semibold text-sm">{reviews[0].name}</div>
                    <div className="text-text-muted text-[10px] md:text-xs">{reviews[0].car}</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* Remaining reviews — 1 per cell, 2 cols on mobile */}
          {reviews.slice(1).map((review, i) => (
            <ScrollReveal key={i} animation="fade-up" delay={(i + 1) * 80} className="col-span-1 md:col-span-3 lg:col-span-5">
              <div className="card p-4 md:p-6 h-full flex flex-col">
                <Stars count={review.rating} />
                <p className="text-text-secondary text-xs md:text-sm leading-relaxed mt-2 md:mt-3 mb-3 md:mb-4 flex-1 line-clamp-4 md:line-clamp-none">&ldquo;{review.text}&rdquo;</p>
                <div className="flex items-center justify-between border-t border-border pt-2 md:pt-3">
                  <div>
                    <div className="text-text font-semibold text-xs md:text-sm">{review.name}</div>
                    <div className="text-text-muted text-[10px]">{review.car}</div>
                  </div>
                  <span className="badge badge-light text-[8px] md:text-[10px] tracking-wider uppercase hidden sm:inline-flex">{review.platform}</span>
                </div>
              </div>
            </ScrollReveal>
          ))}
