...
export default async function ServicesPage() {
  ...
  const { services, contacts } = content;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {services.map((service, i) => (
              <article key={service.title} className="card p-8">
                <div className="flex items-start gap-5 mb-5">
                  <div className={`w-14 h-14 bg-gradient-to-br ${GRADIENTS[i % GRADIENTS.length]} rounded-2xl flex items-center justify-center shrink-0 shadow-lg`}>
          ...
          <div className="card p-10 mt-16 text-center">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <a href={contacts.whatsappUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-primary text-white font-semibold px-8 py-3.5 rounded-xl hover:bg-primary-light transition-colors">
              ...
              <a href={`tel:${contacts.phoneRaw}`} className="inline-flex items-center gap-2 text-text font-semibold hover:text-primary transition-colors">
                {contacts.phone}
              </a>
            </div>
          </div>
